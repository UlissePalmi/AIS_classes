...
dates = pd.to_datetime(ls_returns.index.astype(str), format="%Y%m")
fig, ax = plt.subplots(figsize=(12, 5))
ax.plot(dates, cum_ret.values, color="black", linewidth=1)
ax.axhline(1, color="gray", linewidth=0.8, linestyle="--")
ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{x:.1f}x"))
ax.set_xlim(dates[0], dates[-1])
ax.grid(True, linestyle="--", alpha=0.4)
ax.set_title("Cumulative Return --- Industry Momentum Long-Short Strategy")
plt.tight_layout()
plt.savefig("latex/cumret_chart.png", dpi=150)
plt.close()
print("Cumulative return chart saved to latex/cumret_chart.png")
...
with open(tex_path, "w", encoding="utf-8") as f:
    ...
    f.write("    \\centering\n")
    f.write("    \\includegraphics[width=\\linewidth]{cumret_chart.png}\n")
    f.write("    \\caption{Cumulative Return - Industry Momentum Long-Short Strategy}\n")
    f.write("\\end{figure}\n\n")
    f.write("\\begin{figure}[htbp]\n")
    f.write("    \\centering\n")
